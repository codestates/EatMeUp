...
const socialLogin = async (req, res) => {
  try {
    ...
    await userCheck.save();
    //2-3. 쿠키,응답보내주기
    res.status(isNewUser ? 201 : 200).cookie("accessToken", accessToken, {
    ...
    res.status(isNewUser ? 201 : 200).cookie("refreshToken", refreshToken, {
      domain: process.env.NODE_ENV ? "api.eatmeup.me" : "localhost",
      path: "/",
      sameSite: "none",
      httpOnly: true,
    ...
    return res.status(isNewUser ? 201 : 200).json({
      ...
      isAuth: true,
      message: isNewUser
        ? "[social] signup and login success"
